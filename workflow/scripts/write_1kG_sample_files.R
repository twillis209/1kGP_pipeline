...
if (grepl("hg38", snakemake@input[["ped"]])) {
  ...
  for (x in c("eur", "afr", "amr", "eas", "sas")) {
    fwrite(dat[Superpopulation == toupper(x), .(SampleID)], file = snakemake@output[[x]], sep = "\t", col.names = FALSE, row.names = FALSE, quote = FALSE)
  }

  fwrite(dat[, .(SampleID)], file = snakemake@output[["all"]], sep = "\t", col.names = FALSE, row.names = FALSE, quote = FALSE)
  ...
  for (x in c("eur", "afr", "amr", "eas", "sas")) {
    fwrite(merged_dat[Superpopulation == toupper(x), .(ID)], file = snakemake@output[[x]], sep = " ", col.names = FALSE, row.names = FALSE, quote = FALSE)
  }

  fwrite(merged_dat[, .(ID)], file = snakemake@output[["all"]], sep = " ", col.names = FALSE, row.names = FALSE, quote = FALSE)
} else {
